...
The [`homelab-documentation`](https://github.com/JoeKun/homelab-documentation) repository contains a number of ready-to-use configuration files for FreeBSD. Some of those configuration files can be used as-is, whereas others may only require lightweight personal customizations.

Clone the `homelab-documentation` repository to `/`.

```console
# cd /
# git clone https://github.com/JoeKun/homelab-documentation.git
```
...
```console
# cd
# ln -s ../homelab-documentation/freebsd-server/git/home/my_username/.gitconfig
# mkdir -p .config
# cd .config
# ln -s ../../homelab-documentation/freebsd-server/git/home/my_username/.config/git
```
...
# for file_name in zlogin zlogout zprofile zshenv zshrc; \
  do \
      ln -s ../../../homelab-documentation/freebsd-server/zsh/usr/local/etc/${file_name}; \
  done
```
...
# for file_name in .zshenv .zshrc; \
  do \
    ln -s ../homelab-documentation/freebsd-server/zsh/root/${file_name}; \
  done
```
...
# for file_name in vimrc colors plugin indent syntax filetype.vim; \
  do \
    ln -s ../../../../homelab-documentation/freebsd-server/vim/usr/local/etc/vim/${file_name}; \
  done
```
...
# cd /usr/local/etc
# rm -f vimpagerrc
# ln -s ../../../homelab-documentation/freebsd-server/vim/usr/local/etc/vimpagerrc
```
...
```console
# cd /homelab-documentation/freebsd-server/password-prompts/patches
...
Feel free to check the sample configuration files for `mutt` in the [`freebsd-server/mutt`](mutt/home/my_username/.mutt) directory of the `homelab-documentation` repository.
