...
For example, you can define:
 ...
 - `tcp_services` as a list of TCP services that should be reachable from the internet;
 - `udp_services` as a list of UDP services that should be reachable from the internet;
...
You can inspect the `abusive_hosts` persistent table set up above in `/etc/pf.conf` using the `pfctl` command line tool.
...
You may also optionally install and set up `sshguard` to protect your server from brute-force attacks against SSH and other services.
...
[^3]: An equivalent patch for `sshguard.conf` can also be found in the `homelab-documentation` repository at the following location: `freebsd-server/firewall/patches/sshguard/sshguard-use-pf-backend.diff`.
...
```console
# cat << EOF > /usr/local/etc/rc.conf.d/sshguard
# /usr/local/etc/rc.conf.d/sshguard: system configuration for sshguard
...
Network file sharing with NFSv3 requires special firewall configuration. Assuming you set up NFS following [this guide](freebsd-network-file-sharing-nfs.md#firewall-impact), you will need to make the following updates to `pf.conf`.

First, add a few new variables at the top, right before the `lan_subnet` one.
...
Then, in the rules section, add a new rule to allow incoming traffic from your local area network for these services.
...
This rule should be added right before the one to allow all incoming traffic from personal cloud servers.
...
You should now be able to mount an NFS file share from this server on any other machine in your local area network.