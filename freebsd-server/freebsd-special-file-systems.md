...
For example, `openjdk17` requires `fdescfs` mounted on `/dev/fd` and `procfs` mounted on `/proc`. [^1]

[^1]: See [`openjdk17`’s `pkg-message`](https://www.freshports.org/java/openjdk17#message) for more context.
