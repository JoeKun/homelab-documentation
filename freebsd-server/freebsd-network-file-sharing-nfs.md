...
Setting up network file sharing with NFSv3 on FreeBSD requires enabling a few services that are available in the FreeBSD base system.

 - `nfsd`: remote NFS server;
 - `mountd`: service that handles remote NFS mount requests;
 - `lockd`: NFS file locking daemon;
 - `statd`: host status monitoring daemon;
 - `rpcbind`: directory service for RPC program numbers.

## Firewall impact

In order to allow mounting network file shares on another machine, you need to make sure that any relevant service that supports the usage of network file shares won’t be blocked by a firewall.

While `rpcbind` and `nfsd` use fixed ports, 111 and 2049 respectively, `mountd`, `lockd` and `statd` use dynamically assigned ports, which are harder to deal with when a firewall is in the mix.

So in this guide, we’ll be pinning those services to specific fixed ports:

 - `mountd`: port 2050;
 - `lockd`: port 2051;
 - `statd`: port 2052.

For more details on how to configure this correctly, please check the NFS section in [this other guide about using `pf` as a firewall on FreeBSD](freebsd-firewall.md#firewall-configuration-for-nfs).


## Enable required services

Enable the above mentioned services on your file server running FreeBSD. [^1]
...
We’ll assume that specific machine has a stable IP address, and is reachable using the host name `my_client`. You can make that work by simply adding a new entry mapping that IP address to the name `my_client` in `/etc/hosts`. Alternatively, some network gateways like the ones from UniFi allow you to set this up using a local DNS entry.
