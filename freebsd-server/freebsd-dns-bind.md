...
In the `options` section:
 ...
 - below the comment that mentions *For named to be accessible to the network*, comment out the `listen-on` directive.
...
A good baseline for the `named.conf.options` configuration file can be found in the [`freebsd-server/dns`](dns/var/named/usr/local/etc/namedb/named.conf.options) directory of the `homelab-documentation` repository.
...
A good baseline for the `named.conf.logging` configuration file can be found in the [`freebsd-server/dns`](dns/var/named/usr/local/etc/namedb/named.conf.logging) directory of the `homelab-documentation` repository.
...
As an alternative to making those changes manually in your `named.conf` configuration file, you may apply those changes with just a few commands, assuming you [fetched this homelab documentation](freebsd-command-line-tools.md#fetch-configuration-files) in `/homelab-documentation`.

```console
# cd /var/named/usr/local/etc/namedb
# patch --posix -p1 -i /homelab-documentation/freebsd-server/dns/patches/modularize-named.conf.diff
```


## Local configuration for `named`

A good baseline for the `named.conf.local` configuration file can be found in the [`freebsd-server/dns`](dns/var/named/usr/local/etc/namedb/named.conf.local) directory of the `homelab-documentation` repository.
...
This kind of configuration requires that you carefully set up the corresponding DNS glue records with your domain name registrar.
...
If you are also using BuddyNS for your secondary name servers, feel free to refer to the [`buddyns-secondary-nameservers.zone-include`](dns/var/named/var/db/namedb/primary/include/buddyns-secondary-nameservers.zone-include) file in the `homelab-documentation` repository.
...
```console
# cat << EOF > /usr/local/etc/rc.conf.d/named
# /usr/local/etc/rc.conf.d/named: system configuration for named
...
This can be achieved by including `named` in `altlog_proglist` in `/etc/rc.conf.d/syslogd`. Here’s an example of what this file might look like.
...
Since that is quite error prone, an easier approach is to leverage the script [`named-reset-dynamic-zone-files`](dns/opt/local/bin/named-reset-dynamic-zone-files). Assuming you [fetched this homelab documentation](freebsd-command-line-tools.md#fetch-configuration-files) in `/homelab-documentation`, you can install this script as follows:

```console
# mkdir -p /opt/local/bin
# cd /opt/local/bin
# ln -s ../../../homelab-documentation/freebsd-server/dns/opt/local/bin/named-reset-dynamic-zone-files
```
