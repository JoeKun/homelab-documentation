...
Assuming you [fetched this homelab documentation](freebsd-command-line-tools.md#fetch-configuration-files) in `/homelab-documentation`, you may create this skeleton with just a few commands.

```console
# cd /usr/local/etc
# ln -s ../../../homelab-documentation/freebsd-server/unprivileged-user-skeleton/usr/local/etc/skel
```
...
This may be a good time to set up SSH keys however you see fit.
...
For a comprehensive configuration file for `git`, you may want to have another copy of the [`homelab-documentation`](https://github.com/JoeKun/homelab-documentation) repository, dedicated for your new unprivileged user, and with the right ownership.
...
% for file_name in .zshenv .zshrc; \
  do \
    ln -s homelab-documentation/freebsd-server/zsh/home/my_username/${file_name}; \
  done
```
