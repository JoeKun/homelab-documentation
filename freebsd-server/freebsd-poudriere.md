...
In this procedure, the approach followed will be to set up a dedicated virtual machine for `poudriere`, so it can be upgraded to a future major version of FreeBSD ahead of your main FreeBSD server, and thus, enable upgrades of your main FreeBSD server with shorter downtime.


## Specific goals

 - Install `poudriere` on a dedicated virtual machine with KVM/`libvirt`.
 - Configure your main FreeBSD server to use exclusively the binary packages produced by `poudriere` running on that virtual machine.
 - Set up storage for built packages in such a way that the virtual machine can remain powered off most of the time, only powering it on when actually using `poudriere` to build packages.
 - Expose `poudriere`’s web interface right from your main FreeBSD server.


## Pre-requisites

First, enable and start NFS-related services on your main FreeBSD server.  
...
See [FreeBSD guest virtual machine on Debian KVM hypervisor host](freebsd-guest-on-debian-kvm-hypervisor.md).
...
Generate a public certificate from the private key.
...
### Share datasets from main FreeBSD server for packages and logs

On your main FreeBSD machine, `my_server`, create a dedicated dataset for `poudriere` data in the `system` pool, as well as children datasets for `packages` and `logs`.
...
Make sure you have an entry for the `my_server` machine in `/etc/hosts` if needed, and mount the `packages` and `logs` network shares inside this new `poudriere` dataset.
...
## Set up build environment for `poudriere`

Create a dedicated jail for `poudriere` to build binary packages for the `arm64` architecture, for the `15.1-RELEASE` version of FreeBSD.
...
Clone a new ports tree for the current quarterly branch from FreeBSD’s official ports tree: `2026Q3`.

```console
# poudriere ports -c -p 2026Q3 -B 2026Q3
```
...
Once the build is complete, set up a symbolic link for the current set of quarterly packages.

```console
# cd /usr/local/poudriere/data/packages
# ln -s my_poudriere-arm64-15-1-2026Q3 my_poudriere-arm64-15-1-quarterly
```


## Configure main FreeBSD server to use private package repository

On your main FreeBSD machine, `my_server`, disable using the default FreeBSD-maintained binary package repositories for `FreeBSD-ports` and `FreeBSD-ports-kmods`.
...
my_poudriere: {
    url: "file:///var/poudriere/packages/my_poudriere-arm64-15-1-quarterly",
...
On the `my_poudriere` virtual machine, update the jail and both of the ports trees.

```console
# poudriere jail -j my_poudriere-arm64-15-1 -u
# poudriere ports -u
# poudriere ports -p 2026Q3 -u
```
...
## Upgrade to a new version of FreeBSD

Following the release of a new version of FreeBSD, you’ll need to create a new jail and rebuild your packages with `poudriere`.

First, upgrade your `my_poudriere` virtual machine to the latest version of FreeBSD, following [the official upgrading instructions](https://www.freebsd.org/releases/15.1R/upgrading/).

After successfully rebooting onto the new version of FreeBSD, create a new dedicated jail for `poudriere` to build binary packages for the `arm64` architecture, for the `15.1-RELEASE` version of FreeBSD.
...
Build packages using your new jail.
...
Assuming the new packages built using the new jail are satisfactory, you may want to clean up existing packages, distribution files and logs associated with the old jail.

```console
# poudriere pkgclean -A -j my_poudriere-arm64-15-0 -p 2026Q3
# poudriere logclean -j my_poudriere-arm64-15-0 -p 2026Q3 -a
```

Then, you can delete the old jail itself.

```console
# poudriere jail -d -j my_poudriere-arm64-15-0
```

Lastly, there are a number of other small artifacts associated with that old jail that can also be removed manually.

```console
# cd /usr/local/poudriere/data
# rm -R -f packages/my_poudriere-arm64-15-0-2026Q3
# rm -f packages/my_poudriere-arm64-15-0-quarterly

# cd /var/run/poudriere
# rm -f lock-poudriere-shared-jail_start_my_poudriere-arm64-15-0-2026Q3.flock
# rm -f lock-poudriere-shared-jail_start_my_poudriere-arm64-15-0-2026Q3.pid
```


## Switch to a new quarterly ports branch

Clone a new ports tree for the new quarterly branch from FreeBSD’s official ports tree: `2026Q4`.

```console
# poudriere ports -c -p 2026Q4 -B 2026Q4
```
...
```console
# cd /usr/local/poudriere/data/packages
# rm -f my_poudriere-arm64-15-1-quarterly
# ln -s my_poudriere-arm64-15-1-2026Q4 my_poudriere-arm64-15-1-quarterly
```
...
```console
# poudriere pkgclean -A -j my_poudriere-arm64-15-1 -p 2026Q3
# poudriere distclean -p 2026Q3 -f /usr/local/etc/poudriere.d/pkglist
# poudriere logclean -j my_poudriere-arm64-15-1 -p 2026Q3 -a
```

Then, the old ports tree itself can be removed.

```console
# poudriere ports -d -p 2026Q3
```
...
```console
# cd /usr/local/poudriere/data
# rm -R -f packages/my_poudriere-arm64-15-1-2026Q3
# rmdir cache/my_poudriere-arm64-15-1-2026Q3
# rmdir .m/my_poudriere-arm64-15-1-2026Q3

# cd /var/run/poudriere
# rm -f lock-poudriere-shared-json_jail_my_poudriere-arm64-15-1-2026Q3.flock
# rm -f lock-poudriere-shared-json_jail_my_poudriere-arm64-15-1-2026Q3.pid
# rm -f lock-poudriere-shared-jail_start_my_poudriere-arm64-15-1-2026Q3.flock
# rm -f lock-poudriere-shared-jail_start_my_poudriere-arm64-15-1-2026Q3.pid
```